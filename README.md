...
**Регистровый файл - 18 регистров.** Регистры общего назначения: `AX`, `BX`, `CX`, `DX`. Указатели: `IP`, `SP`, `BP` (базовый указатель, участвует в `LEAVE` и косвенной адресации со смещением). Индексные: `SI`, `DI`. Сегментные константы: `CS`, `DS`, `SS`. Флаги: `ZF` (ноль), `OF` (переполнение 16-бит), `CE` (критическая ошибка, блокирует конвейер).

---

## Система команд (ISA v2.5)

24 инструкции в 5 категориях, которые реально пишет программист.

| Категория | Команды |
|---|---|
| Передача данных | `MOV`, `PUSH`, `POP`, `LEAVE` |
| Арифметика | `ADD`, `SUB`, `MUL`, `DIV`, `INC`, `DEC`, `NEG` |
| Битовая логика | `AND`, `OR`, `XOR`, `NOT` |
| Управление потоком | `CMP`, `JMP`, `JCX`, `JZ`, `LOOP`, `CALL`, `RET` |
| Ввод/вывод | `INT`, `END` |

На деле в таблице диспетчеризации `self.coms` 26 записей: сверх этих 24 туда зашиты `r_stream_in` и `p_stream_out` - обработчики, на которые переключает `INT 0`/`INT 1`. Формально в парсере они ничем не отличаются от обычной команды, так что их можно вписать в код и напрямую, но задуманы они как внутренняя часть механизма прерываний, а не как часть ISA, поэтому в таблицу их не выношу.
...
- Косвенная адресация через `[BX]`, `[SI]`, `[DI]`, `[BP]`: содержимое регистра интерпретируется MMU как смещение в своем сегменте - `[BX]`/`[SI]`/`[DI]` смотрят в DS, `[BP]` смотрит в SS. Только для `[BP]` можно добавить смещение прямо в скобках: `[BP+4]`, `[BP-2]`.
- `LEAVE`: пишет `SP = BP`, то есть схлопывает стековый кадр подпрограммы одной командой, без ручных PUSH/POP. Пока без пары `ENTER` - кадр приходится собирать вручную через ADD/SUB SP.
- `DIV`: целочисленное деление с усечением к нулю, как аппаратный x86 `IDIV` (`-7 / 2 = -3`, а не `-4`).
- `ADD`/`SUB` разрешены и для `SP` - единственное исключение из защиты системных регистров, нужно чтобы вручную выделять место под локальные переменные в стековом кадре.
...
**Указатели** (`IP`, `SP`, `BP`). IP при сбросе ставится на 342 (начало CS). Инкремент IP происходит сразу после фазы Fetch, до декодирования - так шина адреса освобождается к следующему такту. Команды ветвления просто перезаписывают уже инкрементированный IP новым адресом. SP стартует на 171 и растет вниз: PUSH - декремент потом запись, POP - чтение потом инкремент. SP - системный регистр, но с исключением: `ADD SP` и `SUB SP` разрешены, чтобы вручную подвинуть вершину стека и выделить место под локальные переменные кадра, все остальные команды (MOV, INC и так далее) в SP по прежнему не пускают.
...
**Регистр `BP`** - базовый указатель для стековых кадров. На нем держится команда `LEAVE` - восстановление SP из BP при выходе из подпрограммы, освобождающее место, занятое локальными переменными. Косвенный доступ через `[BP]` смотрит в SS, а не в DS, как остальные регистры-указатели, и это единственный регистр, для которого работает адресация со смещением: `[BP+4]`, `[BP-2]`. Полноценный `ENTER` (автоматическое создание кадра одной командой) пока не сделан - кадр собирается вручную через `ADD`/`SUB SP`.
...
Система из 24 инструкций Тьюринг-полна - достаточно для любого вычислимого алгоритма, от сортировки до рекурсии.
...
**Подпрограммы.** `CALL` сохраняет текущий (уже инкрементированный) IP на стек, затем прыгает. `RET` читает адрес возврата со стека и пишет прямо в IP без промежуточных регистров - баг с затиранием `AX` из v1.8 исправлен именно так. `LEAVE` одной командой пишет `SP = BP`, то есть освобождает всё, что было выделено под локальные переменные кадра, не заставляя программиста руками считать сколько раз делать POP. Пары `ENTER` пока нет, кадр (сохранение старого BP, `ADD SP` под переменные) собирается вручную.

**Косвенная адресация.** Парсер в `computer.py` детектирует `[REG]` или `[REG+n]`/`[REG-n]` через `_ptr_addr`, извлекает имя регистра и смещение, получает адрес и обращается к `self.memory`. `BX`/`SI`/`DI` при этом всегда смотрят в DS, а `BP` - в SS, это единственный регистр, для которого поддержано смещение в скобках. Каждый косвенный запрос проходит через `MMU`.
...
Продвинутый архитектурный и защитный слой. Полная интеграция сегментного MMU с динамической трансляцией адресов - программа больше не знает абсолютных адресов, только смещения внутри своего сегмента. Cookie-сессии заменены на изоляцию через заголовок `X-Client-Id`, решена проблема гонки данных при деплое в Docker-контейнер. Реализован изолированный контроль границ стека и защита сегмента кода. Добавлен механизм непрерывного контроля системных инвариантов (`check_invariants`). Новая инструкция `JZ`. Архитектурная поддержка стековых кадров: регистр `BP` заработал по назначению, добавлена команда `LEAVE`, косвенная адресация расширена до `[BP+n]`/`[BP-n]`, `ADD`/`SUB SP` разрешены как единственное исключение из защиты системных регистров.
...
Ядро покрыто 109 юнит-тестами (`test_computer.py`, pytest): MMU и трансляция адресов, reset/restart, вся система команд с косвенной адресацией, инварианты (I-1 - I-5), граничные случаи вроде деления на ноль, stack overflow/underflow и попыток записи в защищенные регистры.
